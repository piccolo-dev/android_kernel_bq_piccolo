...
#endif
#if defined(CONFIG_AUDIO_CODEC_FLORIDA)// || defined(CONFIG_AUDIO_CODEC_WM8998_SWITCH)
extern void snd_jack_report_florida(int status, int type);
#endif

#if defined(CONFIG_AUDIO_CODEC_WM8998_SWITCH) //defined(CONFIG_AUDIO_CODEC_FLORIDA)
extern void snd_jack_report_wm8998(int status, int type);
#endif
...
inline void arizona_extcon_report(struct arizona_extcon_info *info, int state)
{
	...
	if (state == BIT_HEADSET){
		pr_debug("At %d In (%s),this is a headset\n",__LINE__, __FUNCTION__);
		snd_jack_report_florida(3, 455);
	}
	else if (state == BIT_HEADSET_NO_MIC){
		pr_debug("At %d In (%s),this is a headphone\n",__LINE__, __FUNCTION__);
		snd_jack_report_florida(1, 455);
	}
	else{
		pr_debug("At %d In (%s), wrong, the state is error \n",__LINE__, __FUNCTION__);
		snd_jack_report_florida(0, 455);
	}
#endif
	...
	if (state == BIT_HEADSET){
		pr_debug("At %d In (%s),this is a headset\n",__LINE__, __FUNCTION__);
		snd_jack_report_wm8998(3, 455);
	}
	else if (state == BIT_HEADSET_NO_MIC){
		pr_debug("At %d In (%s),this is a headphone\n",__LINE__, __FUNCTION__);
		snd_jack_report_wm8998(1, 455);
	}
	else{
		pr_debug("At %d In (%s), wrong, the state is error \n",__LINE__, __FUNCTION__);
				 ...
				 unsigned long event, void *data)
{
	...
	int *blank;
	struct arizona *arizona  =
		container_of(self, struct arizona, fb_notif);
	pr_debug("At %d In (%s)\n",__LINE__, __FUNCTION__);
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
			arizona ) { 
		blank = evdata->data;
		if (*blank == FB_BLANK_UNBLANK)			
			arizona->arizona_is_suspend =0;
		else if (*blank == FB_BLANK_POWERDOWN)			
			arizona->arizona_is_suspend =1;
	}
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
	...
	int ret, i;
	//pr_debug("At %d In (%s), now arizona_is_suspend=%d\n",__LINE__, __FUNCTION__,arizona->arizona_is_suspend);
	if(arizona->arizona_is_suspend == 1)
		{
	//pr_debug("At %d In (%s),report a power key here\n",__LINE__, __FUNCTION__);//should add to here,0505
	input_report_key(info->input,116, 1);
	input_sync(info->input);
	input_report_key(info->input,116, 0);// KEY_POWER
	input_sync(info->input);
	}
...
static int arizona_extcon_probe(struct platform_device *pdev)
{
	...
	__set_bit(116, info->input->keybit);
	//__set_bit(116, info->input->evbit);
	...
	#if defined(CONFIG_AUDIO_CODEC_WM8998_SWITCH)
	#if defined(CONFIG_FB)
	arizona->fb_notif.notifier_call = fb_notifier_callback;
	pr_debug("At %d In (%s), will register notifier\n",__LINE__, __FUNCTION__);
	ret = fb_register_client(&arizona->fb_notif);
	if (ret)
		pr_debug("At %d In (%s),Unable to register fb_notifier: %d\n",__LINE__, __FUNCTION__,ret);			
	#endif
	#endif
...
err_rise_wake:
	...
	#if defined(CONFIG_AUDIO_CODEC_WM8998_SWITCH)
	#if defined(CONFIG_FB)
	if (fb_unregister_client(&arizona->fb_notif))
		pr_debug("At %d In (%s),Error occurred while unregistering fb_notifier.\n",__LINE__, __FUNCTION__);			
	#endif
	#endif
err_rise:
