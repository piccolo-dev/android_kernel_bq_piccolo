...
//++++>
int msm_audrx_init_wm8998(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_ignore_suspend(dapm, "HPOUTFB1");
	...
	snd_soc_dapm_ignore_suspend(dapm, "IN2A");
			  ...
			  unsigned int Fref, unsigned int Fout)
{
	...
	printk("%s, codec=%s, fll_id=%d, source=%d, Fref=%d, Fout=%d\n", __func__, 
		codec->name, fll_id, source, Fref, Fout);
	switch (fll_id) {
	case WM8998_FLL1:
...
#if defined(CONFIG_AUDIO_CODEC_WM8998_SWITCH) //defined(CONFIG_AUDIO_CODEC_FLORIDA) 
void snd_jack_report_wm8998(int status, int mask)
{
...
EXPORT_SYMBOL(snd_jack_report_wm8998);
#endif
...
#if defined(CONFIG_AUDIO_CODEC_WM8998_SWITCH)
	printk("%s, ldo_enable=%d, ena_ldo=%d, reset=%d, irq_gpio=%d\n", __func__, gpio_get_value(arizona->pdata.ldo_enable), 
		gpio_get_value(arizona->pdata.ena_ldo), gpio_get_value(arizona->pdata.reset), 
		gpio_get_value(arizona->pdata.irq_gpio));
#else
	printk("%s, ldo_enable=%d, ena_ldo=%d, reset=%d, i2c_addr=%d, irq_gpio=%d\n", __func__, gpio_get_value(arizona->pdata.ldo_enable), 
		gpio_get_value(arizona->pdata.ena_ldo), gpio_get_value(arizona->pdata.reset), 
